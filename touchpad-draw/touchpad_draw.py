...
단축키 (드로잉 모드 중):
    ...
    Enter       화면의 기호들을 텍스트로 확정 (또는 2.5초 멈추면 자동)
...
기호를 그리고 약 1초 멈추면 인식되어 회색 잉크와 라벨로 화면에 남는다.
여러 기호를 이어 그린 뒤 Enter(또는 2.5초 대기)로 텍스트 확정.
...
RECOG_PAUSE = 1.0            # 이 시간(초)만큼 획이 없으면 기호 하나로 확정
COMMIT_PAUSE = 2.5           # 이 시간 멈추면 화면의 기호들을 텍스트로 확정
...
class TouchpadReader(threading.Thread):
    ...
    def __init__(self, out_queue):
        ...
        self._hookproc = HOOKPROC(self._mouse_hook)
    ...
    def _mouse_hook(self, ncode, wparam, lparam):
        # 드로잉 모드 동안 모든 마우스 이벤트(커서 이동/클릭)를 삼킨다
        if ncode >= 0 and self.drawing:
            return 1
...
class OverlayApp:
    ...
    def _default_status(self):
        ...
        return ("기호 그리고 멈추면 인식 · Enter(또는 2.5초 대기)로 텍스트 확정   |   "
    ...
    def _finish_symbol(self):
        ...
        self._sym_seq += 1
        self.canvas.addtag_withtag(tag, "ink")
    ...
    def _poll(self):
        ...
        if (not self.dialog_open and self.expr and not self.strokes
                ...
                and now - self.last_contact > COMMIT_PAUSE):
            self._commit_expr()
